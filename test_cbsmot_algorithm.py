...
from cbsmot_algorithm import (
    haversine, distance_absolute, average_speed, 
    delta_time, Stop, cbsmot
)
...
class TestCBSMoT(unittest.TestCase):
    ...
    def test_cbsmot_with_single_trajectory(self):
        """Test that CB-SMoT returns empty list when only one trajectory is provided"""
        # Create a mock trajectory
        trajectory = MagicMock()
        trajectory.point = MagicMock()
        trajectory.datetime = datetime(2020, 1, 1, 12, 0, 0)
        
        # Call the function with single trajectory
        result = cbsmot([trajectory], 1, 60)
        
        # The result should be an empty list (need at least 2 points to form a stop)
        self.assertEqual(result, [])
    
    def test_cbsmot_with_moving_trajectories(self):
        """Test that CB-SMoT doesn't detect stops when all trajectories are moving"""
        # Create mock trajectories that are far apart (moving)
        trajectories = []
        base_time = datetime(2020, 1, 1, 12, 0, 0)
        
        # Create 10 trajectories 100 meters apart, 10 seconds apart
        for i in range(10):
            trajectory = MagicMock()
            point = MagicMock()
            # Set x, y coordinates to simulate movement (100m between each point)
            point.x = -46.633309 + (i * 0.001)  # Approximately 100m per 0.001 degree
            point.y = -23.550520
            trajectory.point = point
            trajectory.datetime = base_time + timedelta(seconds=i*10)
            trajectories.append(trajectory)
        
        # Call the function with moving trajectories
        # max_average_speed = 1 m/s, min_time = 60 seconds
        result = cbsmot(trajectories, 1, 60)
        
        # The result should be an empty list (no stops detected)
        self.assertEqual(result, [])
